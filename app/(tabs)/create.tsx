...
const CreateScreen = () => {
  ...
  const saveTransaction = async () => {
    ...
    const res = await TransactionController.store(obj);
    if (res.status === 200) {
      setTransaction({
        id: 0,
        user_id: user?.id ?? 0,
  ...
  useEffect(() => {
    (async () => {
      const { data: cats }: { data: schema.Category[] } =
    ...
    <SafeAreaView className="relative min-h-full px-2 pb-8 dark:bg-dark-900 bg-light-100">
      <ScrollView className="h-full" showsVerticalScrollIndicator={false}>
        <View className="fixed top-0 flex-row items-center justify-between">
          <Button variant="link" onPress={() => naviagtion.goBack()}>
            <ChevronLeft color={user?.dark_mode === 1 ? "white" : "black"} />
          </Button>
          <Button variant="link" onPress={saveTransaction}>
